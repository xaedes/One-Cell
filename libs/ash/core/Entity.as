package ash.core {
	...
	public class Entity {
		...
		internal var disabledComponents:Dictionary;
		...
		public function Entity(name:String = "") {
			...
			disabledComponents = new Dictionary();
			if (name) {
				_name = name;
			} else {
		...
		public function disable(componentClass:Class):Entity {
			if (components[componentClass]) {
				disabledComponents[componentClass] = components[componentClass];
				remove(componentClass);
			}
			return this;
		}
		
		public function enable(componentClass:Class):Entity {
			if (disabledComponents[componentClass]) {
				var obj:Object = disabledComponents[componentClass];
				delete disabledComponents[componentClass];
				add(obj);
			}
			return this;
		}
